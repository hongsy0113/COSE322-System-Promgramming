...
int main()
{
	...
	// when the number of ports is under what you typed
		...
		printf("the number of ports is under what you intended!\n"); 	 
...
void client_creator(void *num) // thread function
{
	...
	{
	printf("Client : Can't open stream socket for port %d\n", *port_num);
	exit(0);
	}
	printf("%d ", *port_num);
	...
	printf("\n%s write completed!", fname);

	if(close(client_fd) == -1) // close client socket
	{
		printf("\nclose failed for %d", *port_num);
	}	
